    ...
    .then(result => {
      ...
      if (result.length >= 2 && result.length <= 10) {
        const countryList = result
          .map(
            ({ name, flags }) =>
              `<li class="country-list__item"><img class="country-list__img" src="${flags.svg}" alt="${flags.alt}" width="40px" height="auto"/><p>${name.official}</p></li>`
          )
          .join('');
      ...
      if (result.length === 1) {
        const countryInfo = result
          .map(
            ({ name, flags, languages, capital, population }) =>
              `<img class="country-info__img" src="${flags.svg}" alt="${
                flags.alt
              }" width="70px" height="auto"/><p class="country-info__name">${
                name.official
              }</p><p class="country-info__capital"><span class="country-info__span">Capital:</span > ${capital}</p><p class="country-info__population"><span class="country-info__span">Population:</span> ${population}</p><p class="country-info__languages"><span class="country-info__span">Languages: </span>${Object.values(
                languages
              )}</p>`
          )
          .join('');
